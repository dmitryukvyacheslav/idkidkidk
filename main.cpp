...
bool LoadTextureFromBmpBuffer(const char* buffer, int len, GLuint* out_texture, int* out_width, int* out_height)
{
    ...
    int size = 4 * image_width * image_height;
...
int main(int, char**)
{
    ...
    SDL_Window* window = SDL_CreateWindow("npaBocygue cJIeno", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 640, 480, window_flags);
    ...
    while (!done)
    {
        ...
        // 2. Show a simple window that we create ourselves. We use a Begin/End pair to create a named window.
        {
            ...
            ImGuiWindowFlags flags = 0 + ImGuiWindowFlags_NoCollapse + ImGuiWindowFlags_NoResize;
            ...
            if (ImGui::BeginPopup("EmptyTree")) {
                ImGui::Text("Tree is empty.", minmaxval);
                if (ImGui::Button("OK"))
                    ImGui::CloseCurrentPopup();
                ImGui::EndPopup();
            }
            ...
            if (ImGui::Button("Add")) {                            // Buttons return true when clicked (most widgets return true when edited/activated)
                if (t->lookup(key) == nullptr) {
                    t->add(key);
                    LoadTextureFromBmpBuffer((const char*)t->render_dfs(), t->m_rd_len, &tex, &tex_w, &tex_h);
                    t->free_renderdata();
                }
                else
            ...
            ImGui::PopStyleColor();
            ImVec2 res = { (float)tex_w, (float)tex_h };
            static ImVec2 pan_offset = { 0,0 };
            ImVec2 pos = ImGui::GetMousePos();
            ImVec2 wpos = ImGui::GetWindowPos();
            ImVec2 wsize = ImGui::GetWindowSize();
            ImVec2 relpos = { pos.x - wpos.x, pos.y - wpos.y };
